...
function LogBackend(startupTime, config, emitter, logger) {
  ...
  emitter.on('flush', function(timestamp, metrics) {
    ...
    write(ts, pluck(metrics.timer_data, 'count_ps', 'rate'), 'count/sec');
    write(ts, pluck(metrics.timer_data, 'count'), 'count');
    write(ts, pluck(metrics.timer_data, 'count_90'), 'count');
    write(ts, pluck(metrics.timer_data, 'lower'), 'ms');
    write(ts, pluck(metrics.timer_data, 'upper'), 'ms');
    write(ts, pluck(metrics.timer_data, 'upper_90'), 'ms');
    write(ts, pluck(metrics.timer_data, 'median', 'average'), 'ms');
    write(ts, pluck(metrics.timer_data, 'mean_90', 'average_90'), 'ms');
...
function pluck(obj, prop, rename) {
  var o = Object.create(null);
  _.each(obj, function(v, n) {
    o[n + '.' + (rename || prop)] = v[prop];
  });
  return o;
}
